...
// Function to check our file size (Human readable format)
...
void print_file_info(const char *path, const struct stat *file_stat) {
    ...
    const char *user_name = (pwd != NULL) ? pwd->pw_name : "UNKNOWN";
    const char *group_name = (grp != NULL) ? grp->gr_name : "UNKNOWN";

    printf("%-40s\t%-12s\t%-10s\t%-10s\t", path, size_str, user_name, group_name);
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2 || argc > 3) {
        ...
        printf("This directory listing tool displays files, sizes, groups, users and attributes of files.\n");
        printf("For a basic directory listing use magpiedir.\n");
        printf("\nVersion:" version "\n");
        return 1;
    }
    ...
    if (pattern && match_count == 1 && pattern_was_expanded(pattern, first_match)) {
        printf("\nTotal size of files: (skipped due to unquoted pattern)\n");
    } else {
